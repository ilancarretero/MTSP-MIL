...
from utils.explanation import generate_fold_explanations
...
def run_cross_validation(df: pd.DataFrame,
    ...
    # Define paths for folders with the original videos .mp4
    video_dirs = [
        "./FIRST_FOLDER_WITH_VIDEOS",
        "./SECOND_FOLDER_WITH_VIDEOS"
    ]
    ...
    for fold in folds:
        ...
        # Definir la raíz de salida para las explicaciones dentro de results_root/run_tag
        explanations_root = os.path.join(base_dir, "explanations") 
        
        generate_fold_explanations(
            model=model,
            val_df=val_df,          # Pasamos el DF de validación con 'SAMPLES' y 'embeddings'
            video_dirs=video_dirs,  # Dónde buscar los .mp4
            output_root=explanations_root, # Donde guardar (se creará fold_X_val/...)
            fold=fold,
            device=device,
            label_col=label_col,
            top_k=5                 # Número de frames a extraer
        )
        # ---------------------------------------------------------
...
def main():
    ...
    p.add_argument("--excel_path", type=str, 
                   default="./EXCEL_WITH_CV_PARTITIONS.xlsx",
                   help="Path to the Excel file containing labels and fold info.")
    
    p.add_argument("--emb_dir_1", type=str, default="./FIRST_FOLDER_WITH_EMBEDDINGS",
                   help="First directory containing embeddings.")
    p.add_argument("--emb_dir_2", type=str, default="./SECOND_FOLDER_WITH_EMBEDDINGS",
    ...
    p.add_argument("--results_root", type=str, default="./RESULTS_FOLDER")
